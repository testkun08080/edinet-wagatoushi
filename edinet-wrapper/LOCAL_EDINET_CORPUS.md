...
DOC_TYPE=quarterly YEAR=2019 MONTH=3 bash scripts/edinet_corpus.sh
```
...
```bash
DOC_TYPE=quarterly YEAR=2020 MONTH=6 bash scripts/edinet_corpus.sh
```
...
for m in 1 6 12; do
  DOC_TYPE=quarterly YEAR=2019 MONTH=$m bash scripts/edinet_corpus.sh
done
```
...
| Run corpus (one month) | `DOC_TYPE=... YEAR=... MONTH=... bash scripts/edinet_corpus.sh` |
...
- 実行スクリプト: `edinet-wrapper/scripts/edinet_corpus.sh`
- 実処理: `edinet-wrapper/scripts/prepare_edinet_corpus.py`
