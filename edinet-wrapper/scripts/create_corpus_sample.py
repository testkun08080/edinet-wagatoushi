...
使い方:
  ...
  uv run python scripts/create_corpus_sample.py

  uv run python scripts/create_corpus_sample.py --year 2024
  uv run python scripts/create_corpus_sample.py --sec_codes 7974 9424 7203 --year 2025
  uv run python scripts/create_corpus_sample.py --edinet_codes E02367 E04473 E02144
  uv run python scripts/create_corpus_sample.py --types annual,quarterly --year 2025

  # --- data-set から自動で企業を選ぶ場合（従来の create_corpus_sample_6companies.py 相当）---
  # 有価証券・四半期・半期（・大量保有）が揃う企業から6社を選び、四半期・半期のみコピー（出力は -sample なし）
  uv run python scripts/create_corpus_sample.py --auto_pick

  # 四半期・半期の両方に存在する企業のみから6社
  uv run python scripts/create_corpus_sample.py --auto_pick --auto_pick_both
